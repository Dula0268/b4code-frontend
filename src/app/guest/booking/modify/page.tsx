...
interface RoomOption {
  id: string; name: string; details: string; imageSrc: string; isBase: boolean; extraPerNight?: number
}

const ROOM_OPTIONS: RoomOption[] = [
  { id: "deluxe", name: "Deluxe Suite", details: "King Bed · 45 sqm · City View", imageSrc: "/images/booking/room-deluxe.png", isBase: true },
  { id: "executive", name: "Executive Suite", details: "King Bed · 55 sqm · Ocean View", imageSrc: "/images/booking/room-executive.png", isBase: false, extraPerNight: APP_CONFIG.executiveExtra },
]

const ORIGINAL_DATA = { bookingId: "#RES-882910", checkIn: "2024-06-12", checkOut: "2024-06-15", nights: 3, total: 15000, guests: 2 }
...
function formatCurrency(amount: number) { return `${APP_CONFIG.defaultCurrency} ${amount.toLocaleString("en-US")}` }
function fmtShort(iso: string) { if (!iso) return ""; return new Date(`${iso}T00:00:00`).toLocaleDateString("en-US", { month: "long", day: "numeric" }) }
function fmtLong(iso: string) { if (!iso) return ""; return new Date(`${iso}T00:00:00`).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }) }
function nightsBetween(from: string, to: string) { const diff = new Date(to).getTime() - new Date(from).getTime(); return Math.max(0, Math.round(diff / 86_400_000)) }
...
function useModifyReservationLogic() {
  ...
  return {
    originalData, checkIn, handleCheckIn, checkOut, setCheckOut,
    guests, handleIncrementGuests: () => setGuests(g => Math.min(8, g + 1)), handleDecrementGuests: () => setGuests(g => Math.max(1, g - 1)),
    roomId, setRoomId, newNights, dateValid, submitting, errorMsg,
    additionalDue, newTotal, roomUpgrade, guestFeeTotal,
    handleConfirmChanges, handleCancelModifications: () => router.back()
  }
}
...
function ModifyContent() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <GuestTopbar />
      <main className="flex-1 pt-20 pb-12" style={{ background: "color-mix(in srgb, var(--gray-5) 60%, white)" }}>
        <div className="max-w-[1020px] mx-auto px-4 pt-6">

          {/* Breadcrumb */}
          <nav className="flex items-center gap-1.5 text-xs mb-5" style={{ color: "var(--gray-3)" }}>
            <Link href="/" className="flex items-center gap-1 no-underline hover:text-[var(--brand-primary)] transition-colors"><Home size={13} /> Home</Link>
            <ChevronRight size={12} />
            <Link href="/guest/booking/my-bookings" className="no-underline hover:text-[var(--brand-primary)] transition-colors">Reservations</Link>
            <ChevronRight size={12} />
          ...
          {logic.errorMsg && (
            <div className="mb-5 p-4 rounded-xl flex items-center gap-2 bg-red-50 text-red-700 border border-red-200">
              <AlertTriangle size={18} /><p className="text-sm font-semibold">{logic.errorMsg}</p>
            </div>
          )}
          ...
          <header className="flex flex-col sm:flex-row sm:items-start justify-between mb-6 gap-3">
            <div>
              <h1 className="text-[1.75rem] font-black leading-tight" style={{ color: "var(--fg)" }}>Modify Your Reservation</h1>
              <p className="text-sm mt-1" style={{ color: "var(--gray-3)" }}>
                Booking {logic.originalData.bookingId} · <span className="font-semibold" style={{ color: "var(--state-success)" }}>Confirmed</span>
              </p>
            </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-0 border rounded-xl overflow-hidden mb-3" style={{ borderColor: "var(--border)" }}>
                      <div className="px-4 py-3 border-r" style={{ background: "color-mix(in srgb, var(--gray-5) 60%, white)", borderColor: "var(--border)" }}>
                        ...
                        <p className="text-sm font-semibold" style={{ color: "var(--gray-2)" }}>{fmtShort(logic.originalData.checkIn)} – {fmtLong(logic.originalData.checkOut)}</p>
                      </div>
                      <div className="px-4 py-3 bg-white">
                        <p className="text-[0.625rem] font-bold uppercase tracking-widest mb-1" style={{ color: "var(--brand-primary)" }}>New Selection</p>
                        <p className="text-sm font-semibold" style={{ color: "var(--fg)" }}>{fmtShort(logic.checkIn)} – {fmtLong(logic.checkOut)}</p>
                      </div>
                    </div>
                    {/* Date pickers */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {[
                        ...
                        <div key={id}>
                          <label className="block text-[0.6875rem] font-semibold uppercase tracking-wide mb-1.5" style={{ color: "var(--gray-3)" }}>{label}</label>
                          <div className="relative">
                            <CalendarDays size={14} className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: "var(--brand-primary)" }} />
                            <input id={id} type="date" value={value} min={min} onChange={(e) => onChange(e.target.value)}
                    ...
                    {logic.dateValid ? (
                      <p className="text-xs font-medium mt-2.5 flex items-center gap-1" style={{ color: "var(--state-success)" }}>
                        <CheckCircle2 size={13} /> {logic.newNights} night{logic.newNights !== 1 ? "s" : ""} selected — availability confirmed.
                      </p>
                    ) : (
                      ...
                      <div className="px-4 py-3 bg-white flex items-center gap-3">
                        <div>
                          ...
                          <div className="flex items-center gap-3">
                            <button onClick={logic.handleDecrementGuests} className="w-7 h-7 rounded-full border flex items-center justify-center text-sm font-bold transition-colors cursor-pointer" style={{ borderColor: "var(--border)", color: "var(--gray-2)" }}>−</button>
                            <span className="text-sm font-bold min-w-[64px] text-center" style={{ color: "var(--fg)" }}>{logic.guests} Adults</span>
                            <button onClick={logic.handleIncrementGuests} className="w-7 h-7 rounded-full border flex items-center justify-center text-sm font-bold transition-colors cursor-pointer" style={{ borderColor: "var(--border)", color: "var(--gray-2)" }}>+</button>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <div className="ps-card overflow-hidden">
                ...
                <div className="px-6 py-5 flex flex-col gap-4">
                  <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
                    ...
                    <div><p className="text-sm font-bold text-amber-700">Limited Availability</p><p className="text-xs text-amber-600">Only 2 Executive Suites left on your selected dates.</p></div>
                  </div>
                  {ROOM_OPTIONS.map((room) => {
                    ...
                    return (
                      <div key={room.id} onClick={() => logic.setRoomId(room.id)}
                        ...
                        style={{ borderColor: selected ? "var(--brand-primary)" : "var(--border)", background: selected ? "color-mix(in srgb, var(--brand-primary) 3%, white)" : "white" }}>
                        <div className="relative w-[90px] h-[65px] rounded-lg overflow-hidden flex-shrink-0" style={{ background: "var(--gray-5)" }}>
                        ...
                        {selected && room.isBase ? (
                          <span className="flex-shrink-0 text-[0.6875rem] font-bold text-white px-3 py-1.5 rounded-full" style={{ background: "var(--brand-primary)" }}>CURRENT</span>
                        ) : !room.isBase ? (
                          <span className="flex-shrink-0 text-sm font-bold" style={{ color: selected ? "var(--brand-primary)" : "var(--gray-3)" }}>+{formatCurrency(room.extraPerNight!)} / night</span>
                        ) : null}
                      </div>
                    )
                  })}
                ...
                <ul className="flex flex-col gap-2">
                  {MODIFICATION_POLICY.map((point) => (
                    <li key={point} className="flex items-start gap-2 text-xs" style={{ color: "var(--gray-2)" }}>
                      <span className="flex-shrink-0 mt-0.5" style={{ color: "var(--brand-primary)" }}>•</span>{point}
                    </li>
                  ))}
                </ul>
              </div>
