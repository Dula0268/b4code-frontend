...
function SubmitReviewContent() {
  ...
  if (submitted) {
    return (
      <div className="flex items-center justify-center px-4 pt-20" style={{ background: "transparent" }}>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        <div className="ps-card p-5 sm:p-6">
          <div className="flex items-center gap-3 mb-5">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "color-mix(in srgb, var(--brand-secondary) 15%, white)", border: "1px solid color-mix(in srgb, var(--brand-secondary) 20%, transparent)" }}>
          ...
          <div className="flex flex-col items-center py-4">
            ...
            {overallRating === 0 && <p className="text-xs mt-3" style={{ color: "var(--gray-4)" }}>Tap a star to rate</p>}
          </div>
        </div>
        ...
        <div className="ps-card p-5 sm:p-6">
          ...
          <p className="text-xs mb-4" style={{ color: "var(--gray-3)" }}>Share what made your stay special (or areas to improve). Minimum 20 characters.</p>
          <textarea
            id="review-text"
            value={reviewText}
          ...
          <div className="flex justify-between items-center mt-2">
            {reviewText.length > 0 && reviewText.length < 20 && <p className="text-[0.625rem] font-semibold" style={{ color: "var(--state-warning)" }}>{20 - reviewText.length} more characters needed</p>}
            <span className="text-[0.625rem] ml-auto font-medium" style={{ color: "var(--gray-4)" }}>{reviewText.length} / {MAX_REVIEW_LENGTH}</span>
          </div>
        </div>
        ...
        <div className="ps-card p-5 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color: "var(--gray-3)" }}>Up to {MAX_PHOTOS} photos (optional)</p>
            </div>
            <span className="text-[0.625rem] font-black px-2.5 py-1 rounded-full border" style={{ background: "color-mix(in srgb, var(--gray-5) 40%, white)", borderColor: "var(--border)", color: "var(--gray-3)" }}>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
            {photos.map(photo => (
              <div key={photo.id} className="relative aspect-square rounded-xl overflow-hidden border group" style={{ borderColor: "var(--border)" }}>
                <Image src={photo.url} alt={photo.name} fill className="object-cover" />
                <button type="button" onClick={() => removePhoto(photo.id)} className="absolute top-1 right-1 w-6 h-6 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer" style={{ background: "rgba(0,0,0,0.6)" }}><X size={12} className="text-white" /></button>
              </div>
            ))}

            {photos.length < MAX_PHOTOS && (
              <button type="button" onClick={() => fileInputRef.current?.click()} className="aspect-square rounded-xl border-2 border-dashed flex flex-col items-center justify-center gap-1 cursor-pointer transition-colors" style={{ borderColor: "var(--border)" }}>
          ...
          <div className="flex gap-2 mt-3">
            <button type="button" onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 px-4 py-2.5 border rounded-xl text-xs font-bold transition-colors cursor-pointer" style={{ borderColor: "var(--border)", color: "var(--gray-2)", background: "color-mix(in srgb, var(--gray-5) 40%, white)" }}><Camera size={14} /> Take Photo</button>
            <button type="button" onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 px-4 py-2.5 border rounded-xl text-xs font-bold transition-colors cursor-pointer" style={{ borderColor: "var(--border)", color: "var(--gray-2)", background: "color-mix(in srgb, var(--gray-5) 40%, white)" }}><ImagePlus size={14} /> Upload Photos</button>
          </div>
        ...
        <div className="pb-4">
          ...
          <button type="submit" disabled={!isFormValid || isSubmitting} className="w-full py-4 rounded-xl text-[0.9375rem] font-black text-white transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2" style={{ background: "var(--brand-primary)" }}>
            {isSubmitting ? <><RefreshCw size={16} className="animate-spin" /> Submitting...</> : "Submit Review"}
          </button>
          <p className="text-[0.6875rem] text-center mt-3 leading-relaxed" style={{ color: "var(--gray-4)" }}>By submitting, you certify this review is based on your genuine experience at this property.</p>
        </div>
      </form>
    </div>
  )
