...
function Dashboard() {
  ...
  const { active, loading } = useActiveBooking()
  const user   = useAuthStore(s => s.user)
  ...
  const nightsDone = Math.max(0, Math.min(active.nights, Math.floor((new Date().getTime() - new Date(active.checkIn).getTime()) / (1000 * 60 * 60 * 24))))
  const nightsTotal = active.nights
  const stayProgress = Math.round((nightsDone / nightsTotal) * 100)
  const stayDates = active.checkInFormatted + " – " + active.checkOutFormatted
  const balanceDue = `LKR ${active.totalPrice.toLocaleString()}`
  const wifiNetwork = `${active.property.replace(/\s+/g, "")}_Guest`
  const wifiPass = `Stay${active.id}!`
          ...
          <div className="relative rounded-[1.5rem] px-6 sm:px-10 py-7 mb-5 overflow-hidden" style={{ background: "var(--brand-primary)" }}>
            ...
            <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              <div>
                ...
                <p className="text-sm text-white/40 mt-1">{active.property} · {active.roomName}</p>
              </div>

              <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                {[
                  { icon: BedDouble,     label: "Room",      value: active.roomName  },
                  { icon: Clock,         label: "Check-in",  value: active.checkInFormatted },
                  { icon: CalendarClock, label: "Check-out", value: active.checkOutFormatted },
              ...
              <div className="ps-card p-5 sm:p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    ...
                    { icon: MessageSquare, label: "Staff Chat",   sub: "We reply fast", href: `/guest/messages?type=staff&bookingId=${active.id}`,  bg: "bg-blue-50",   border: "border-blue-100",   iconCls: "text-blue-600"   },
                    ...
                    { icon: Pencil,        label: "Write Review", sub: "Share feedback", href: `/guest/reviews?propertyId=${active.propertyId}`, bg: "bg-purple-50", border: "border-purple-100", iconCls: "text-purple-600" },
              ...
              <div className="ps-card p-5 sm:p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                  {[
                    ...
                    <Link key={label} href={`/guest/messages?type=staff&bookingId=${active.id}&q=${encodeURIComponent(q)}`} className="flex flex-col items-center gap-2 p-3.5 border rounded-xl transition-all no-underline text-center group" style={{ borderColor: "var(--border)" }}>
                ...
                <Link href={`/guest/messages?type=staff&bookingId=${active.id}`} className="mt-4 w-full py-3 border rounded-xl text-[0.8125rem] font-bold flex items-center justify-center gap-2 no-underline transition-all" style={{ borderColor: "var(--border)", color: "var(--gray-2)" }}>
            ...
            <div className="flex flex-col gap-5">
              <div className="rounded-[1.25rem] p-5 text-white" style={{ background: "var(--brand-primary)" }}>
                ...
                <div className="space-y-3">
                  {[
                    { icon: BedDouble,     label: "Room",      value: active.roomName              },
                    { icon: Clock,         label: "Check-in",  value: `${active.checkInFormatted}, 2:00 PM`   },
                    { icon: CalendarClock, label: "Check-out", value: `${active.checkOutFormatted}, 11:00 AM` },
              ...
              <div className="ps-card p-5">
                ...
                <div className="flex gap-2">
                  ...
                  <Link href={`/guest/messages?type=staff&bookingId=${active.id}`} className="flex-1 flex items-center justify-center gap-2 py-3 border rounded-xl text-xs font-bold transition-all no-underline" style={{ borderColor: "var(--border)", color: "var(--gray-2)" }}><MessageSquare size={14} /> Message</Link>
                </div>
              </div>
              ...
              <div className="ps-card p-5">
                ...
                <Link href={`/guest/reviews?propertyId=${active.propertyId}`} className="w-full py-3 rounded-xl text-[0.8125rem] font-bold flex items-center justify-center gap-2 no-underline transition-colors" style={{ background: "var(--brand-primary)", color: "white" }}><Pencil size={14} /> Write a Review</Link>
              </div>
            </div>
