...
export default function PropertyDetailsPage() {
  ...
  const { selectedProperty, getPropertyById, loading, actionLoading, approveProperty, rejectProperty, markUnderReview } = useAdminPropertiesStore();
  ...
  const handleUnderReview = async () => {
    try {
      ...
      alert(e.message || "Failed to mark under review.");
    }
  };

  const isPending = selectedProperty.status === "Pending" || selectedProperty.status === "PENDING";
  const isUnderReview = selectedProperty.status === "Under Review" || selectedProperty.status === "UNDER_REVIEW";
    ...
    <AdminPageLayout>
      <div className="flex flex-col gap-6 pb-24 relative">
        ...
        <div className="flex items-center gap-1.5 text-sm">
          ...
          <span className="text-[#C05621] font-semibold">{selectedProperty.name}</span>
        </div>
        ...
        <div className="border-b border-[#F0EBE7] pb-6">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className={`px-3 py-1 rounded-full text-[12px] font-bold ${
              isUnderReview ? "bg-[#DBEAFE] text-[#1E40AF]" :
              isPending ? "bg-[#FEF3C7] text-[#92400E]" :
              selectedProperty.status === 'Approved' ? "bg-[#D1FAE5] text-[#065F46]" :
          ...
          <p className="text-[13px] text-[#9E7B6A] mt-1.5 m-0">
            Submitted on {new Date(selectedProperty.submissionDate).toLocaleDateString()} • ID: #{selectedProperty.id}
          </p>
        </div>
