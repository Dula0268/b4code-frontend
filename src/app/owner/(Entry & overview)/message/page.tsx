...
const INITIALS_COLORS = ["#2e7d32", "#1565c0", "#8B6914", "#6a1b9a", "#c62828", "#00695c"];

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function mapConversation(c: any, idx: number): Conversation {
    const parts = (c.guestName || "G").split(" ");
    ...
    return {
        ...
        property: (c.propertyName || "").toUpperCase(),
        time: c.updatedAt || "",
        unread: !!c.unread,
        online: false,
    };
...
export default function MessagePage() {
    ...
    useEffect(() => {
        messageApi.getConversations(ownerId)
            .then((data) => {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const mapped = (data.conversations || []).map((c: any, idx: number) => mapConversation(c, idx));
                setConversations(mapped);
                if (mapped.length > 0) setSelectedConversation(mapped[0].id);
            })
    ...
    useEffect(() => {
        ...
        messageApi.getMessages(selectedConversation)
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            .then((data: any[]) => setChatMessages(data.map((m) => ({
                id: m.id,
                sender: m.senderType === "OWNER" ? "owner" : "guest",
                text: m.content || "",
                time: m.sentAt || "",
                read: !!m.read,
            }))))
