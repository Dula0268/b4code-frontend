...
/* Peak days fallback (used before API responds) */
...
export default function OwnerDashboardPage() {
    ...
    useEffect(() => {
        const fetchDashboard = async () => {
            ...
            try {
                ...
                const data = await dashboardApi.getDashboard(ownerId, calYear, calMonth);
                setDashboardData(data);
            } catch (err) {
                console.error("Failed to fetch dashboard data:", err);
                setError("Could not load dashboard data. Showing cached values.");
            } finally {
                setLoading(false);
            }
        };
        fetchDashboard();
    }, [calYear, calMonth, user?.userId]);
    ...
    const highlightDay = isCurrentMonth ? now.getDate() : -1;
                ...
                <div className="grid grid-cols-6 gap-3 mb-4">
                    {loading
                        ? Array.from({ length: 6 }).map((_, i) => (
                            <div
                                key={i}
                                className={`rounded-xl border px-4 py-3 flex flex-col gap-1 animate-pulse ${
                                    i === 5 ? "border-[#953002] bg-[#953002]" : "border-[#e0e0e0] bg-white"
                                }`}
                            >
                                <div className="h-2 w-3/4 rounded bg-[#e0e0e0]" />
                                <div className="h-8 w-1/2 rounded bg-[#e0e0e0] mt-1" />
                            </div>
                        ))
                        ...
                        <div className="bg-white border border-[#e0e0e0] rounded-xl p-4">
                            ...
                            <div className="flex justify-between items-center mb-3">
                                ...
                                <span className="text-[12px] font-medium text-[#828282]">
                                    {dashboardData?.availabilityPreview?.occupancyPercent != null
                                        ? `${dashboardData.availabilityPreview.occupancyPercent}% Occupancy`
                                        : loading ? "—" : "0% Occupancy"}
                                </span>
                            </div>
                            ...
                            <div className="mt-4 pt-3 border-t border-[#f0f0f0]">
                                ...
                                <div className="flex flex-col gap-1.5">
                                    {(dashboardData?.availabilityPreview?.keyHighlights?.length
                                        ? dashboardData.availabilityPreview.keyHighlights
                                        : [
                                            { color: "#e74c3c", message: "No peak data yet" },
                                            { color: "#e67e22", message: "Check room schedules" },
                                        ]
                                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                                    ).map((h: any, i: number) => (
                                        <div key={i} className="flex items-center gap-2 text-[12px] text-[#4f4f4f]">
                            ...
                            <table className="w-full border-collapse text-[12px]">
                                ...
                                <tbody>
                                    ...
                                    {currentReservations.map((r: any) => (
                                        <tr key={r.id ?? (r.guestName || r.name)} className="border-b border-[#f5f5f5]">
                                            {/* Guest */}
                                            <td className="py-2.5 px-3 align-middle">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="font-semibold text-[13px] text-[#1d1d1d]">
                                                        {r.guestName ?? r.name}
                                                    </span>
                                                </div>
                                            </td>
                ...
                <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="bg-white border border-[#e0e0e0] rounded-xl px-5 py-4 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <span className="text-[18px] font-extrabold text-[#1d1d1d]">{dashboardData?.newReviews?.value ?? "—"}</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-white border border-[#e0e0e0] rounded-xl px-5 py-4 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <span className="text-[18px] font-extrabold text-[#1d1d1d]">{dashboardData?.maintenanceAlerts?.value ?? "—"}</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-white border border-[#e0e0e0] rounded-xl px-5 py-4 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <span className="text-[18px] font-extrabold text-[#2e7d32]">{dashboardData?.growth?.value ?? "—"}</span>
                        </div>
                    </div>
                </div>
            </main>
