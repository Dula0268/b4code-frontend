...
export default function WeeklyCalendarPage() {
    ...
    useEffect(() => {
        propertiesApi.listProperties(ownerId, 1, 20)
            .then((data) => {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const mapped = (data.properties || []).map((p: any, i: number) => ({
                    id: p.id,
                    name: p.name,
                    icon: i === 0 ? "home" : i === 1 ? "building" : "mountain",
                    active: i === 0,
                }));
                setProperties(mapped);
                if (mapped.length > 0) setActiveProperty(mapped[0].id);
            })
    ...
    useEffect(() => {
        ...
        availabilityApi.getWeeklyCalendar(activeProperty, fmt(baseDate))
            .then((data) => {
                ...
                (data.days || []).forEach((day: any) => {
                    ...
                    if (status === "BOOKED") {
                        bookings[key] = { type: "booked", guest: day.guestName || "" };
                    } else if (status === "BLOCKED") {
                        bookings[key] = { type: "blocked" };
                    } else {
                        bookings[key] = { type: "available", price: day.price ? String(day.price) : "" };
                    }
                });
                setMockBookings(bookings);
            })
            .catch((err) => console.error("Failed to load availability:", err));
    }, [activeProperty, baseDate]);
