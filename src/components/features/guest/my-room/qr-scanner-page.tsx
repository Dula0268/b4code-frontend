...
import { AlertCircle, Camera, CheckCircle2, ExternalLink, HelpCircle, RefreshCw, X } from "lucide-react"
...
function useQrScannerLogic() {
  ...
  const setQRContext = useOrderContextStore((state) => state.setQRContext)
  const setLoading = useOrderContextStore((state) => state.setLoading)
  const setError = useOrderContextStore((state) => state.setError)
  ...
  const closeCamera = useCallback(() => {
    ...
    setLoading(false)
    setError(null)
  }, [setError, setLoading, stopCamera])
  ...
  const validateQrCode = useCallback(
    async (qrId: string) => {
      ...
      try {
        const response = await api.post("/qr/validate", {}, { params: { qrId } })
    ...
    [router, setError, setLoading, setQRContext, stopCamera]
  )

  const startScanLoop = useCallback(() => {
    if (!("BarcodeDetector" in window)) {
      setPhase("error")
      setErrorMsg("This browser does not support QR scanning.")
      return
    }
    ...
    const loop = async () => {
      ...
      try {
        ...
        if (codes.length > 0) {
          ...
          stopCamera()
          setTimeout(() => {
            void validateQrCode(codes[0].rawValue)
          }, 350)
          return
        }
      } catch {
        // Ignore transient decode errors and continue scanning.
      }
...
export default function QrScannerPage() {
  ...
  return (
    <>
      <div
        className="min-h-screen pt-24 pb-16 flex flex-col items-center justify-center px-4"
      ...
      >
        <div className="w-full max-w-[420px] flex flex-col items-center text-center">
          ...
          <h1
            className="text-[2.125rem] font-black tracking-tight leading-tight mb-3"
            style={{ color: "var(--fg)", fontSize: "clamp(1.5rem, 5vw, 2.125rem)" }}
          >
          ...
          <div className="relative mb-10">
            <div
              className="rounded-3xl p-8 flex items-center justify-center w-[280px] h-[280px]"
          ...
          <div
            className="w-[280px] border rounded-full py-2.5 flex items-center justify-center gap-2"
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-5 px-6">
            ...
            {phase === "scanning" && (
              <div className="w-full max-w-[350px] flex flex-col items-center gap-4">
                <div className="relative w-full aspect-square rounded-2xl overflow-hidden bg-black border border-white/10">
                  ...
                  <div className="absolute inset-0 pointer-events-none">
                    {["tl", "tr", "bl", "br"].map((position) => (
                      <div
                        key={position}
                        className={`absolute w-10 h-10 border-[3px] border-amber-400 ${
                          position === "tl"
                            ? "top-6 left-6 rounded-tl-md border-r-0 border-b-0"
                            : position === "tr"
                              ? "top-6 right-6 rounded-tr-md border-l-0 border-b-0"
                              : position === "bl"
                              ? "bottom-6 left-6 rounded-bl-md border-r-0 border-t-0"
                ...
                <button
                  onClick={closeCamera}
                  className="text-white/40 hover:text-white/70 text-sm transition-colors cursor-pointer flex items-center gap-1.5"
                >
