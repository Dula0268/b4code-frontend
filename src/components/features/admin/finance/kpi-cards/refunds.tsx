...
export default function RefundsCard() {
  ...
  const refundAmount = summary.pendingRefunds ?? summary.totalRefunds ?? 0;
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm flex flex-col justify-between h-[136px] min-w-0 overflow-hidden">
      <div className="flex items-start justify-between mb-2">
        <h3 className="text-sm text-gray-500 font-medium truncate">Total Refunds</h3>
      ...
      <div className="flex flex-col gap-1 min-w-0">
        ...
        <p className="text-xs text-gray-400 font-medium mt-1 truncate">Approved refunds total</p>
      </div>
    </div>
  );
}
