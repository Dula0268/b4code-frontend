...
export default function GuestGlobalOrdersProvider({ children }: { children: React.ReactNode }) {
  ...
  // Per-order reconnect delay (ms), exponential backoff starting at 1s, capped at 30s
  ...
  useEffect(() => {
    function openStream(orderId: string) {
      activeStreams.current.add(orderId);

      const guestSessionId = useGuestSessionStore.getState().sessionId;
      const url = guestSessionId
        ? `${api.defaults.baseURL}/orders/${orderId}/stream?guestSessionId=${encodeURIComponent(guestSessionId)}`
        : `${api.defaults.baseURL}/orders/${orderId}/stream`;
      ...
      eventSource.addEventListener("status-update", (event) => {
        try {
          ...
          // Connection is healthy again — reset backoff
          reconnectDelays.current.delete(orderId);
          ...
          if (mappedStatus === "delivered" || mappedStatus === "cancelled") {
            ...
            reconnectDelays.current.delete(orderId);
            const timer = reconnectTimers.current.get(orderId);
            if (timer) {
              clearTimeout(timer);
              reconnectTimers.current.delete(orderId);
            }
          }
        } catch (err) {
      ...
      eventSource.onerror = () => {
        ...
        // Don't reconnect if the order has since reached a terminal state
        const storeCurrentOrder = useOrderStore.getState().currentOrder;
        const storeOrderHistory = useOrderStore.getState().orderHistory;
        const fullOrderId = `#ORD-${orderId}`;
        const relevantOrder =
          storeCurrentOrder?.id === fullOrderId
            ? storeCurrentOrder
            : storeOrderHistory.find((o) => o.id === fullOrderId);
        if (relevantOrder && ["delivered", "cancelled"].includes(relevantOrder.currentStatus)) {
          reconnectDelays.current.delete(orderId);
          return;
        }

        if (eventSource.readyState === EventSource.CLOSED) {
          const delay = reconnectDelays.current.get(orderId) ?? 1000;
          ...
          reconnectTimers.current.set(orderId, timer);
          reconnectDelays.current.set(orderId, Math.min(delay * 2, 30000));
        }
      };
    }
    ...
    activeOrders.forEach((_status, orderId) => {
      if (!activeStreams.current.has(orderId) && !reconnectTimers.current.has(orderId)) {
        openStream(orderId);
      }
    });

    return () => {
      // We do not cleanup active streams/timers on unmount so they stay persistent across route changes
